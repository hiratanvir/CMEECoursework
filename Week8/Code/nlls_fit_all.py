...
for i,g in grouped:
    ...
    x_vals = g['ConTemp']    #x is you temp data, use temp in celcius
    y_vals = g['OriginalTraitValue'] #is the y value, use the non-log trait data
    ...
    data = pd.DataFrame({"ID": i, "A": A1, "B": B1, "C": C1, "D": D1, "AIC": AIC , "chi-squared": chi_squared, "r-squared":r2, "status":status}, index=[0])
    cubic_dF = cubic_dF.append(data)

cubic_dF.to_csv('../Results/cubic_report.csv', columns=['ID', 'A', 'B', 'C', 'D','AIC','chi-squared','r-squared','status'], sep=',', encoding='utf-8')
...
for i,g in grouped:
    ...
    non_log_y = g['OriginalTraitValue']
...
schoolfield_dF.to_csv('../Results/schoolfield_report.csv', columns=['ID','B0','E','Eh','El','Th','Tl','AIC','chi-squared','r-squared','status'], sep=',', encoding='utf-8')
