...
one_group = subset_dF.loc[subset_dF['uniqueID']<3]

for i, group in one_group.groupby("uniqueID"):

    #define x and y axis for e parameter estimate plot
    x = one_group['1/kT']
    y = one_group['log_TraitValues']
    ...
    max_y = one_group['log_TraitValues'].max(axis=0) #Maximum y value but does not index it
    ...
    B0 = float(one_group.loc[one_group['Temp(kel)']==(find_nearest(Temp,value)), 'StandardisedTraitValue'])
    E = E_estimate[0]
    Eh = Eh_estimate[0]
    El = E/2
    Th = float(xvalues[idy])
    Tl = one_group['1/kT'].min(axis=0)

    #Adding empty columns which will store starting parameter into dataframe for NLLS fitting
    one_group = one_group.assign(B0=B0, E=E, Eh=Eh, El=El, Th=Th, Tl=Tl)
    one_group.to_csv('one_group.csv', sep=',', encoding='utf-8')
