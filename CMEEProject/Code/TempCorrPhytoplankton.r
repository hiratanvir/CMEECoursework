...
unique_data <- unique_data[ ! unique_data$uniqueID %in% c(136), ]
#unique_data <- unique_data[!grepl("136", unique_data$uniqueID),]
...
write.table(HighTemp, "../Results/HighTemp_Phytoplankton.csv", sep = ",", row.names = FALSE)
...
LT <- ggplot(data = LowTemp, aes(x = AverageVolume, y = LowTemp_GR)) +
  ...
  geom_point(col="darkblue") + theme_bw()+ theme(axis.title.x=element_blank(),
                                     axis.title.y=element_blank()) + ggtitle("3°C")
...
MT <- ggplot(data = MidTemp, aes(x = AverageVolume, y = MidTemp_GR)) +
  ...
  geom_point(col="orange") + theme_bw()+ theme(axis.title.x=element_blank(),
                                     axis.title.y=element_blank()) + ggtitle("16.85°C")
...
HT <- ggplot(data = HighTemp, aes(x = AverageVolume, y = HighTemp_GR)) +
  ...
  geom_point(col="red") + theme_bw() + theme(axis.title.x=element_blank(),
                                     axis.title.y=element_blank()) + ggtitle("23.05°C")

print(HT)

pdf("../Results/plots/phytoplankton_scaling.pdf") 
plots <- grid.arrange(LT, MT, HT, top = textGrob("Phytoplankton Temperatures Corrected Plots",gp=gpar(fontsize=13,font=3)),
...
LL <- ggplotRegression(lm_LT) +            
  ...
  ylab(expression(paste("log(Growth rate, ", s^{-1},") at 3°C"))) + theme_bw()
print(LL)
ggsave("../Results/plots/supplementary/LowTemp_phyto.pdf")
...
MM <- ggplotRegression(lm_MT) +          
  ...
  ylab(expression(paste("log(Growth rate, ", s^{-1},") at 16.85°C"))) + theme_bw()
print(MM)
ggsave("../Results/plots/supplementary/MidTemp_phyto.pdf")
...
HH <- ggplotRegression(lm_HT) +         
  ...
  ylab(expression(paste("log(Growth rate, ", s^{-1},") at 23.05°C"))) + theme_bw()
print(HH)
ggsave("../Results/plots/supplementary/HighTemp_phyto.pdf") 
...
data_long <- data_long %>% mutate (Temp = ifelse(Temperature == 'LowTemp_GR', '3', 
                                                 ifelse(Temperature == 'MidTemp_GR', '16.85',
                                                        ifelse(Temperature == 'HighTemp_GR', '23.05', NA))))

data_long$Temp <- as.numeric(as.character(data_long$Temp))
write.table(data_long, "../Results/Phytoplankton_all.csv", sep = ",", row.names = FALSE)
...
combined_plot <- ggplot(data_long, aes(x = AverageVolume, y = GrowthRate, color=Temperature)) + 
  ...
  ylab(expression(paste("log(Growth rate, ", s^{-1},")"))) + ggtitle("Phytoplankton Temperature-Corrected Regressions") + theme_bw()

print(combined_plot)
ggsave("../Results/plots/phytoplankton_slopes.pdf")
