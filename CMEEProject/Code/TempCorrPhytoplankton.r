...
LT <- ggplot(data = LowTemp, aes(x = AverageVolume, y = LowTemp_GR)) +
  ...
  geom_point(col="darkblue") +  xlab(expression(paste("log(Volume, ", m^{3},")")))+ ylab(expression(paste("log(Growth rate, ", s^{-1},") at 3°C")))
...
MT <- ggplot(data = MidTemp, aes(x = AverageVolume, y = MidTemp_GR)) +
  ...
  stat_poly_eq(formula = y~x, eq.with.lhs=FALSE, 
               aes(label = paste("hat(italic(y))","~`=`~",..eq.label..,"~~~", ..rr.label.., sep = "")), 
               parse = TRUE, label.y.npc = 0.2) +         
  geom_point(col="orange") +  xlab(expression(paste("log(Volume, ", m^{3},")")))+ ylab(expression(paste("log(Growth rate, ", s^{-1},") at 16.85°C")))
...
HT <- ggplot(data = HighTemp, aes(x = AverageVolume, y = HighTemp_GR)) +
  ...
  stat_poly_eq(formula = y~x, eq.with.lhs=FALSE, 
               aes(label = paste("hat(italic(y))","~`=`~",..eq.label..,"~~~", ..rr.label.., sep = "")), 
               parse = TRUE, label.y.npc = 0.2) +         
  geom_point(col="red") +  xlab(expression(paste("log(Volume, ", m^{3},")")))+ ylab(expression(paste("log(Growth rate, ", s^{-1},") at 23.05°C")))
